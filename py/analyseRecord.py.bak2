...
# file=open("./text_test01.txt",'r',encoding='utf-8')
# text=file.read()
# file.close()
...
class TextAnalysis:
    ...
    def Get_English_dict(self):
        ...
        text_English = "".join([word for word in re.findall(r'[\x00-\xff]+', text_test) if word not in stopwords.words()])
    ...
    def Get_Chinese_dict(self):
        ...
        tokens_tag = [(token.word, token.flag) for token in tokens if token.word not in ['，', '。', '？', '！', '！']]
        dict_noun = {a: t.count(a) for a, b in tokens_tag if b in ['n', 'nr', 'ns', 'nt', 'nz']}
        # print("切分后的字典为：",dict_noun)
    ...
    def Get_total_dict(self):
        ...
        text_English_part = "".join(
            [word for word in re.findall(r'[\x00-\xff]+', text_test) if word not in stopwords.words()])
